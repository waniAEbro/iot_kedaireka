...
class M_aluminium extends CI_Model
{
	...
	public function getRows_mf($postData)
	{
		$this->_get_datatables_query_mf($postData);
		if ($postData['length'] != -1) {
			$this->db->limit($postData['length'], $postData['start']);
		}
		$query = $this->db->get();
    ...
    public function countFiltered_mf($postData)
	{
		$this->_get_datatables_query_mf($postData);
		$query = $this->db->get();
		return $query->num_rows();
	}
    private function _get_datatables_query_mf($postData)
	{
		$id_jenis_item = 1;
        $this->db->join('master_warna mwa', 'mwa.kode = mi.kode_warna', 'left');
        $this->db->where('mi.id_jenis_item', $id_jenis_item);
    ...
    public function getdatapaging_mf($num = false, $keyword = '', $perpage = '', $offset = '')
    {
        $id_jenis_item = 1;
        $this->db->join('master_warna mwa', 'mwa.kode = mi.kode_warna', 'left');
        $this->db->where('mi.id_jenis_item', $id_jenis_item);
    ...
    public function getStockAkhirBulan()
    {
        $this->db->join('master_gudang mg', 'mg.id = ds.id_gudang', 'left');
        $this->db->where('mg.jenis_aluminium', 1);
        $res  = $this->db->get('data_counter ds');
        $data = array();
    ...
    public function getQtyOutFppp($id_fppp, $id_item)
    {
        ...
        return $this->db->get('data_stock ds')->row()->tot_out;
    }
    ...
    public function getQtyAwalBulan($tgl)
    {
        ...
        foreach ($res->result() as $key) {
            if (isset($data[$key->id_item][$key->id_gudang][$key->keranjang])) {
                $nilai = $data[$key->id_item][$key->id_gudang][$key->keranjang];
            } else {
                $nilai = 0;
            }
            $data[$key->id_item][$key->id_gudang][$key->keranjang] = $key->qty_in + $nilai;
        }
        return $data;
    }
    ...
    public function getQtyKeluar($tgl,$id)
    {
        ...
        foreach ($res->result() as $key) {
            ...
            $data[$key->id_item][$key->id_gudang][$key->keranjang] = $key->qty_out + $nilai;
        }
        return $data;
    }
}
