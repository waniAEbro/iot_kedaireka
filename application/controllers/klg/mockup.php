...
class Mockup extends CI_Controller
{
    ...
    public function formAdd($param = '')
    {
        ...
        $data['no_fppp']            = str_pad($this->m_fppp->getNoMockup($param), 3, '0', STR_PAD_LEFT) . '/MOCKUP/' . $nama_divisi . '/' . date('m') . '/' . date('Y');
    ...
    public function savefppp($value = '')
    {
        ...
        $this->fungsi->catat($datapost, "Menyimpan Mockup sbb:", true);
        $data['msg'] = "Mockup Disimpan";
        echo json_encode($data);
    }
    ...
    public function savefpppImage($value = '')
    {
        ...
        if (!$this->upload->do_upload('lampiran')) {
            ...
            $this->fungsi->catat($datapost, "Menyimpan Mock Up sbb:", true);
            $data['msg'] = "Mock Up Disimpan";
            echo json_encode($data);
        }
    }
