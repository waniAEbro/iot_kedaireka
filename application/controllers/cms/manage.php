...
class Manage extends CI_Controller
{
	...
	public function menu()
	{
		...
		$data['level'] = $this->m_menu->get_level();
		$data['menu'] = $this->m_menu->get_menu_manage();
		$data['level'] = $this->db->get('master_level');
	...
	public function add_menu()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data['allowed'] = get_options($this->db->get('master_level'), 'id', 'level');
	...
	function configure_def()
	{
		$this->fungsi->check_previleges('config');
		$this->load->library('form_validation');
		$config = array(
			array(
				'field'	=> 'email_manager',
		...
		if ($this->form_validation->run() == FALSE) {
			$data['grupedit'] = $this->db->get('default_conf')->row();
			$this->load->view('cms/manage/konfigurasi_awal', $data);
		} else {
			$datapost = get_post_data(array('email_manager'));
			$this->m_manage->save_conf($datapost);
			$this->fungsi->catat($datapost, "Mengedit grup menu dengan data sbb:", true);
			$data['msg'] = "Konfigurasi Disimpan";
