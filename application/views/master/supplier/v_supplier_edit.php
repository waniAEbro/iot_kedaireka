...
<div class="box-body big">
        ...
        <div class="col-sm-8">
            <select id="id_jenis_item" name="id_jenis_item" class="form-control" style="width:100%" required>
                ...
                <?php foreach ($jenis_item->result() as $valap) :
                    $sel_ji = ($valap->id == $row->id_jenis_item) ? 'selected' : ''; ?>
                    <option value="<?= $valap->id ?>" <?= $sel_ji ?>><?= $valap->jenis_item ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
        ...
        <div class="col-sm-8">
            <select id="id_kabupaten" name="id_kabupaten" class="form-control" style="width:100%" required>
                ...
                <?php foreach ($kabupaten->result() as $valap) :
                    $sel_k = ($valap->id == $row->id_kabupaten) ? 'selected' : ''; ?>
                    <option value="<?= $valap->id ?>" <?= $sel_k ?>><?= $valap->kabupaten ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
    ...
    <div class="form-group">
        ...
        <div class="col-sm-8 tutup">
            <?php
            echo button('send_form(document.faddmenugrup,"master/supplier/show_editForm/","#divsubcontent")', 'Simpan', 'btn btn-success') . " ";
            ?>
        </div>
    </div>
    </form>
