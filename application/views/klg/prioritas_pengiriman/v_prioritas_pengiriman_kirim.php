          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="hidden" class="form-control" size="2" id="id_pengiriman">
              </div> 
            </div>
          </div>
          ...
          <table width="100%" id="tableku" class="table table-bordered" style="font-size: smaller;">
            ...
            <tbody>
              ...
              <tr bgcolor="<?=$notif?>">
                ...
                <td align="center">
                <?php if ($permintaan!=0) { ?>
                <input type="hidden" id="sto_<?=$val->id?>" value="<?=$stok?>"><input type="hidden" id="per_<?=$val->id?>" value="<?=$permintaan?>"><input type="text" class="form-control" value="<?=$permintaan?>" size="2" id="qty_<?=$val->id?>">
                <?php }?>
                </td>
                <td align="center">
                <?php if ($permintaan!=0) { ?>
...
function simpan(id) {
  ...
  if(parseInt(qty)>parseInt(permintaan)){
      $.growl.error({ title: 'Peringatan', message: 'Qty melebihi permintaan!' });
    } else{
      if(parseInt(qty)>parseInt(stk)){
      $.growl.error({ title: 'Peringatan', message: 'Qty melebihi stock!' });
      }else{  
        $.ajax({
            type: "POST",
