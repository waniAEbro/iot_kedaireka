...
function finish() {
  if(confirm('Anda yakin ingin menyelesaikan?'))
  {
    ...
    load_silent("klg/retur_lama/","#content");
  }
}
...
function quotation() {
  if ($('#item').val() !='' && $('#lebar').val() != '' && $('#tinggi').val() != '' && $('#qty').val() !=''  ) 
  {
                  ...
                  $.ajax({
                      type: "POST",
                      url: "<?=site_url('klg/retur_lama/saveretur_lamaDetail')?>",
                      dataType:'json',
                      data: {
...
$("select[name=store]").change(function(){
    ...
    if($(this).val() == "") {
      ...
      $.ajax({
        url      : "<?=site_url('klg/retur_lama/getItemInvoice')?>",
        dataType : "json",
        type     : "POST",
        data     : { "id" : $(this).val() },
...
$("#item").change(function(){
      $.ajax({
        url      : "<?= site_url('klg/retur_lama/getDetailInvoice/')?>",
        dataType : "json",
        type     : "POST",
        data     : { 
          "id" : $(this).val(),
