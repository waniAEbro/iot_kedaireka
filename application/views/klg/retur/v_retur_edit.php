          ...
          <div class="row">
            <div class="col-md-4">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="no_retur" name="no_retur" value="<?=$edit->no_retur?>" placeholder="No Invoice" readonly required>
              </div>              
            </div>
            <div class="col-md-4">
              <div class="form-group">
                  <label>Jenis Retur</label>
                  <select id="jenis_retur" name="jenis_retur" class="form-control" style="width:100%" required disabled>
                    <option value="">-- Select Jenis Retur --</option>
                    <?php foreach ($jenis_retur as $val):?>
                      <?php $select = ($edit->id_jenis_retur == $val->id) ? 'selected' : '' ;?>
                      <option value="<?=$val->id?>" <?=$select?>><?=$val->jenis_retur?></option>
                    <?php endforeach;?>
                  </select>
              </div>              
            </div>
              ...
              <div class="form-group">
                  ...
                  <select id="alasan_retur" name="alasan_retur" class="form-control" style="width:100%" required>
                    ...
                    <?php foreach ($alasan_retur as $val):?>
                      <?php $selected = ($edit->id_alasan_retur == $val->id) ? 'selected' : '' ;?>
                      <option value="<?=$val->id?>" <?=$selected?>><?=$val->alasan_retur?></option>
                    <?php endforeach;?>
                  </select>
              </div>              
            </div>
          ...
          <div class="row">
              ...
              <div class="form-group">
                ...
                <select id="store" name="store" class="form-control" style="width:100%" required>
                  ...
                  <?php foreach ($store as $valap):?>
                    <?php $select = ($edit->id_store == $valap->id) ? 'selected' : '' ;?>
                    <option value="<?=$valap->id?>" <?=$select?>><?=$valap->store?></option>
                  <?php endforeach;?>
                </select>
              </div>              
            </div>
            <div class="col-md-6">
              <div class="form-group">
                ...
                <input type="text" data-date-format="yyyy-mm-dd" class="form-control datepicker" id="tgl_penarikan" value="<?=$edit->tgl_penarikan?>" placeholder="Tanggal Penarikan" required>
              </div>             
            </div>
          </div>
          <div class="row">
            <div class="col-md-12">
              <div class="form-group">
                ...
                <input type="text" class="form-control" id="keterangan" value="<?=$edit->keterangan?>" placeholder="Keterangan">
              </div>
            </div>
          </div>
          <div class="row">
            ...
            <form method="post" class="form-vertical form" role="form" id="formid">
              <div class="box-body">
                ...
                <div class="form-group">
                  ...
                  <input type="hidden" class="form-control" id="id_retur" name="id_retur" value="<?=$edit->id?>"  readonly>
            ...
            <table width="100%" id="tableku" class="table table-bordered table-striped" style="font-size: smaller;">
              ...
              <tbody id="dataTbl">
                ...
                <tr id="output_data_<?=$val->id?>" class="output_data">
                  ...
                  <td width="30%">
                  <?=$val->item?>_<?=$val->tipe?>_<?=$val->lebar?>x<?=$val->tinggi?>_<?=$val->warna?>_<?=$val->bukaan?>
                  </td>
                  <td width="5%">
                    <?=$val->qty?>
                  </td>
                  <td width="30%">
                    <?=$val->item_baru?>_<?=$val->tipe_baru?>_<?=$val->lebar_baru?>x<?=$val->tinggi_baru?>_<?=$val->warna_baru?>_<?=$val->bukaan_baru?>
                  </td>
                  <td width="5%">
                    <?=$val->qty?>
                  </td>
        ...
        <div class="box box-info">
          ...
          <div class="box-body">
            <table width="100%" id="tableku" class="table table-bordered table-striped" style="font-size: smaller;">
              ...
              <tbody id="dataTbl4">
                <?php $j = 1; ?>
                <?php foreach($detail as $valj):
                ?>

                <tr id="output_data_<?=$valj->id?>" class="output_data">
                  <td width="5%" align="center">
                    <a href="javascript:void(0)" onClick="hapus(<?=$valj->id?>)">
                      <i class="fa fa-trash"></i>
                    </a>
                  </td>
                  <td width="30%">
                  <?=$valj->item?>_<?=$valj->tipe?>_<?=$valj->lebar?>x<?=$valj->tinggi?>_<?=$valj->warna?>_<?=$valj->bukaan?>
                  </td>
                  <td width="5%">
                    <?=$val->qty?>
                  </td>
...
$(document).ready(function() {
  ...
  function() 
  {
    ...
    var z = "<option>-- Select Item --</option>";
      $.ajax({
...
function finish() {
  ...
  {
    $.growl.notice({ title: 'Berhasil', message: "Tambah Retur Selesai!"});
...
function save()
{
  ...
  var path = $("#lampiran").val().replace('C:\\fakepath\\', '');
    ...
    $.ajax({
      ...
      $.ajaxFileUpload
          ({
            url:site+'klg/retur/editReturImage',
            secureuri:false,
