    ...
    <div class="col-lg-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">List bomwo</h3>

                <div class="box-tools pull-right">
                    <?php
                    $sesi = from_session('level');
                    if ($sesi == '1' || $sesi == '2') {
                        echo button('load_silent("klg/bomwo/formAdd","#content")', 'Tambah bomwo', 'btn btn-success');
                    } else {
                        # code...
                    }
                    ?>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><?= $row->created ?><input type="hidden" id="id_bomwo_<?= $i ?>" value="<?= $row->id ?>"></td>
                                    ...
                                    <td align="center">
                                        <?php if ($row->lampiran != '') { ?>
                                            <a target="_blank" href="<?= base_url($row->lampiran); ?>" class="btn btn-xs btn-danger">Attachment</a>
                                        <?php } ?>

                                        <?php
                                        if (from_session('id') < 3) {
                                            echo button('load_silent("klg/bomwo/formEdit/' . $row->id . '","#content")', 'Edit khusus', 'btn btn-xs btn-info', 'data-toggle="tooltip" title="Edit"');
                                        }
                                        ?>
                                    </td>
                                </tr>
...
<script type="text/javascript">
    ...
    // function cetak (id) {
    //   var win = window.open("<?php echo base_url('klg/bomwo/cetak/'); ?>/"+id, "_blank");
    //             if (win) {
    ...
    function setFilter() {
        ...
        var tahun = $('#tahun').val();
        load_silent("klg/bomwo/filter/" + id_store + "/" + id_bulan + "/" + tahun + "/", "#content");
    ...
    $(document).ready(function() {
        ...
        function format(id, row, tr) {
            ...
            infoTable = '<table id="infoTable" class="table table-striped" border="1px" style="font-size: smaller;">' +
                ...
                '<th bgcolor="#bfbfbf">Section</th>' +
                '<th bgcolor="#bfbfbf">Warna</th>' +
                '<th bgcolor="#bfbfbf">Qty</th>' +
                '</tr>';

            var id_bomwo = $('#id_bomwo_' + id).val();

            $.ajax({
                    url: "<?= site_url('klg/bomwo/getDetailTabel') ?>",
                    type: 'POST',
                    dataType: 'JSON',
                    data: {
                        id_bomwo: id_bomwo,
                ...
                .done(function(data) {
                    ...
                    for (var i = 0; i < data.detail.length; i++) {
                        ...
                        infoTable += '<tr bgcolor="' + color + '">' +
                            ...
                            '<td><font color="' + fontcolor + '">' + data.detail[i].section + '</font></td>' +
