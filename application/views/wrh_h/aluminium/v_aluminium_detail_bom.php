...
<div class="row">
    <div class="col-lg-12">
        <div class="box box-primary">
                                ...
                                <tr bgcolor="<?= $bgrow ?>">
                                    ...
                                    <td><?= $row->section_ata ?>
                                        ...
                                        <?php if ($row->is_parsial == 1) {
                                            echo button_confirm("Anda yakin hapus parsial item " . $row->section_ata . "-" . $row->section_allure . "?", "wrh_h/aluminium/hapus_parsial/" . $id_fppp . "/" . $row->id_stock, "#content", 'Hapus Parsial', 'btn btn-xs btn-danger', 'data-toggle="tooltip" title="Hapus Parsial"');
                                        }
                                        ?>
                                    </td>
            ...
            <div class="box-footer">
                <?= button_confirm("Anda yakin menyelesaikan stock out?", "wrh_h/aluminium/buat_surat_jalan/" . $id_fppp, "#content", 'Buat Surat Jalan', 'btn btn-success', 'data-toggle="tooltip" title="Buat Surat Jalan"'); ?>
                <?= button_confirm("Anda yakin lanjut ke gudang MF?", "wrh_h/aluminium/stok_out_make_mf/" . $id_fppp, "#content", 'ke Gudang MF', 'btn btn-primary', 'data-toggle="tooltip" title="ke Gudang MF"'); ?>
...
<script type="text/javascript">
    ...
    $(".checkbox").change(function() {
        ...
        // } else {
            $.ajax({
                url: "<?= site_url('wrh_h/aluminium/saveoutcheck/') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    field: fieldname,
            ...
            if (parseInt(qty_gudang) < parseInt(value)) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // if (value == 0) {
                        load_silent("wrh_h/aluminium/stok_out_make/" + id_fppp, "#content");
                        // }
                        // console.log(qtybom);
                        // console.log(value);
                        // gudang(edit_id);
