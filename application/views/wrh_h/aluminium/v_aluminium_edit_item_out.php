        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                ...
                <div class="box-tools pull-right">
                    <?php //echo button('load_silent("wrh_h/aluminium/bon_manual","#content")', 'Kembali', 'btn btn-success');
                    ?>
                </div>
            </div>
            <div class="box-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" data-date-format="yyyy-mm-dd" value="<?= $tgl_aktual ?>" class="form-control" id="tgl_aktual" readonly>
                            </div>
                        </div>
                    </div>
                    <div class="row">
            ...
            <?php
            if (from_session('level') == 1 || from_session('level') == 3 || from_session('level') == 8) { ?>
...
<script type="text/javascript">
    ...
    function update() {
        $.ajax({
            url: "<?= site_url('wrh_h/aluminium/updateSuratJalanBon/') ?>",
            dataType: "json",
            type: "POST",
            data: {
                "id_sj": $("#id_sj").val(),
            ...
            success: function(img) {
                ...
                // load_silent("wrh_h/aluminium/bon_manual/", "#content");
            }
        });
    ...
    $("select[name=item]").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            z = "<option>-- Select --</option>";
            $.ajax({
                url: "<?= site_url('wrh_h/aluminium/optionGetGudangItem') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    "item": $(this).val(),
    ...
    $("select[name=id_gudang]").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            z = "<option>-- Select --</option>";
            $.ajax({
                url: "<?= site_url('wrh_h/aluminium/optionGetKeranjangGudang') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    "item": $('#item').val(),
    ...
    $("select[name=keranjang]").change(function() {
        ...
        $('#lapangan').prop('checked', false);
        $.ajax({
            url: "<?= site_url('wrh_h/aluminium/optionGetQtyKeranjang') ?>",
            dataType: "json",
            type: "POST",
            data: {
                "item": $('#item').val(),
    ...
    function quotation2() {
        ...
        if ($('#id_fppp').val() != '' && $('#item').val() != '' && $('#id_gudang').val() != '' && $('#qty').val() != '') {
            ...
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh_h/aluminium/savebonmanual') ?>",
                    dataType: 'json',
                    data: {
                        'id_sj': $('#id_sj').val(),
                        'id_fppp': $('#id_fppp').val(),
    ...
    function hapus(i) {
        if (confirm('Lanjutkan Proses Hapus?')) {
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh_h/aluminium/deleteItemBonManual') ?>",
                    dataType: 'json',
                    data: {
                        'id': i
                    }
