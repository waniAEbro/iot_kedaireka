    ...
    <div class="col-lg-3">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Input Stock Aluminium</h3>
            </div>
                ...
                <form method="post" class="form-vertical form" role="form" id="formid">
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control datepicker" data-date-format="yyyy-mm-dd" id="aktual" value="<?= date('Y-m-d') ?>" placeholder="Tgl Aktual" autocomplete="off">
                        </div>
                        <div class="form-group">
...
<script language="javascript">
    function finish() {
        if (confirm('Anda yakin ingin menyelesaikan?')) {
            ...
            load_silent("wrh_h/aluminium/finish_stok_in", "#content");
        }
    }
    ...
    function quotation() {
        ...
        if ($('#aktual').val() != '' && $('#item').val() != '' && $('#qty').val() != '' && $('#id_gudang').val() != '' && $('#keranjang').val() != '') {
            ...
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh_h/aluminium/savestokin') ?>",
                    dataType: 'json',
                    data: {
                        'aktual': $('#aktual').val(),
                        'item': $('#item').val(),
    ...
    function hapus(i) {
        if (confirm('Lanjutkan Proses Hapus?')) {
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh_h/aluminium/deleteItemIn') ?>",
                    dataType: 'json',
                    data: {
                        'id': i
                    }
