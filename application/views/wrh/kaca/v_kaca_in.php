    ...
    <div class="col-lg-3">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Input Stock kaca</h3>
            </div>
                            ...
                            <select id="item" name="item" class="form-control" style="width:100%" required>
                                ...
                                <?php foreach ($item->result() as $valap) : ?>
                                    <option value="<?= $valap->id ?>">
                                        <?= $valap->item_code ?>-<?= $valap->deskripsi ?>-<?= $valap->satuan ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
...
<script language="javascript">
    function finish() {
        if (confirm('Anda yakin ingin menyelesaikan?')) {
            ...
            load_silent("wrh/kaca/finish_stok_in", "#content");
        }
    }
    ...
    function quotation() {
        ...
        if ($('#aktual').val() != '' && $('#item').val() != '' && $('#qty').val() != '' && $('#id_divisi').val() != '' && $('#id_gudang').val() != '' && $('#keranjang').val() != '') {
            ...
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh/kaca/savestokin') ?>",
                    dataType: 'json',
                    data: {
                        'aktual': $('#aktual').val(),
                        'item': $('#item').val(),
    ...
    function hapus(i) {
        if (confirm('Lanjutkan Proses Hapus?')) {
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh/kaca/deleteItemIn') ?>",
                    dataType: 'json',
                    data: {
                        'id': i
                    }
