                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    <?php
                                    echo button('load_silent("wrh/kaca/edit_item_stok_out/' . $row->id . '","#content")', 'Edit', 'btn btn-xs btn-info', 'data-toggle="tooltip" title="Edit"');
                                    echo button('load_silent("wrh/kaca/lihat_item_stok_out/' . $row->id . '","#modal")', 'Lihat Item', 'btn btn-xs btn-primary', 'data-toggle="tooltip" title="Lihat Item Stock OUT"');
                                    echo button_confirm("Apakah anda yakin menghapus SJ BON " . $row->no_surat_jalan . "?", "wrh/kaca/deleteSJBon/" . $row->id, "#content", "Hapus", "btn btn-xs btn-danger", "");
                                    ?>
                                    <a target="_blank" href="<?= base_url('wrh/kaca/cetakSjBon'); ?>/<?= $row->id ?>" class="btn btn-xs btn-warning">Cetak Surat Jalan</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
...
<script type="text/javascript">
    ...
    function setFilter(argument) {
        ...
        load_silent("wrh/kaca/bon_manual_diSet/" + tlg1 + "/" + tgl2, "#content");
    ...
    function setCetak() {
        ...
        var url = "<?= site_url('wrh/kaca/bon_manual_diSet_cetak/"+tgl_awal+"/"+tgl_akhir+"') ?>";
