...
<div class="row">
    <div class="col-lg-12">
        <div class="box box-primary">
            ...
            <?php echo button_confirm("Anda yakin menyelesaikan stock out BON?", "wrh/kaca/buat_surat_jalan_bon/", "#content", 'Finish Buat Surat Jalan', 'btn btn-success', 'data-toggle="tooltip" title="Finish Buat Surat Jalan"'); ?>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    $("select[name=id_fppp]").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            $.ajax({
                url: "<?= site_url('wrh/kaca/optionGetMultibrandFppp') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    "id_fppp": $(this).val()
    ...
    $("select[name=item]").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            z = "<option>-- Select --</option>";
            $.ajax({
                url: "<?= site_url('wrh/kaca/optionGetDivisiItem') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    "item": $(this).val()
    ...
    $("select[name=id_divisi]").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            z = "<option>-- Select --</option>";
            $.ajax({
                url: "<?= site_url('wrh/kaca/optionGetGudangDivisi') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    "item": $('#item').val(),
    ...
    $("select[name=id_gudang]").change(function() {
        ...
        if ($(this).val() == "") {
            ...
            z = "<option>-- Select --</option>";
            $.ajax({
                url: "<?= site_url('wrh/kaca/optionGetKeranjangGudang') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    "item": $('#item').val(),
    ...
    $("select[name=keranjang]").change(function() {
        ...
        $('#lapangan').prop('checked', false);
        $.ajax({
            url: "<?= site_url('wrh/kaca/optionGetQtyKeranjang') ?>",
            dataType: "json",
            type: "POST",
            data: {
                "item": $('#item').val(),
    ...
    function quotation2() {
        ...
        if ($('#id_fppp').val() != '' && $('#item').val() != '' && $('#id_divisi').val() != '' && $('#id_gudang').val() != '' && $('#keranjang').val() != '' && $('#qty').val() != '') {
            ...
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh/kaca/savebonmanual') ?>",
                    dataType: 'json',
                    data: {
                        'id_sj': 0,
                        'id_fppp': $('#id_fppp').val(),
    ...
    function hapus(i) {
        if (confirm('Lanjutkan Proses Hapus?')) {
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh/kaca/deleteItemBonManual') ?>",
                    dataType: 'json',
                    data: {
                        'id': i
                    }
