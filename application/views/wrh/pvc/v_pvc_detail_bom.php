    ...
    <div class="col-lg-12">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">List BOM pvc warna</h3>
                ...
                <table width="100%" id="tableku" class="table">
                    ...
                    <tbody>
                        ...
                        foreach ($list_bom->result() as $row) :
                            ...
                            $divisi = $this->m_pvc->getDivisiBomItem($id_jenis_item, $row->id_item);
                            $gudang = $this->m_pvc->getGudangBomItem($id_jenis_item, $row->id_item);
                            $keranjang = $this->m_pvc->getKeranjangBomItem($id_jenis_item, $row->id_item);


                            $qtyTotalOut = $this->m_pvc->getQtyOutFppp($row->id_fppp, $row->id_item);
                            $id_divisi_stock = $this->m_pvc->getQtyTerbanyakStockDivisi($row->id_item);
                            $id_gudang_stock = $this->m_pvc->getQtyTerbanyakStockGudang($row->id_item);
                            $keranjang_stock = $this->m_pvc->getQtyTerbanyakStockKeranjang($row->id_item);
                            $qty_stock = $this->m_pvc->getQtyTerbanyakStockQty($row->id_item);
                            ...
                            if ($qty_gudang < $qtyBOM) {
                                $this->m_pvc->updateIsKurang($row->id_stock);
                                // $this->m_pvc->updatekeMf($row->id_stock, $id_fppp);
                            }
                        ...
                        ?>
                                <tr bgcolor="<?= $bgrow ?>">
                                    ...
                                    <td><?= $row->item_code ?>
                                        <br><?php echo button_confirm("Anda yakin mengirim parsial item " . $row->item_code . "-" . $row->deskripsi . "?", "wrh/pvc/kirim_parsial/" . $id_fppp . "/" . $row->id_stock, "#content", 'Kirim Parsial', 'btn btn-xs btn-default', 'data-toggle="tooltip" title="Kirim Parsial"'); ?>
                                        <br>
                                        <?php if ($row->is_parsial == 1) {
                                            echo button_confirm("Anda yakin mengirim parsial item " . $row->item_code . "-" . $row->deskripsi . "?", "wrh/pvc/hapus_parsial/" . $id_fppp . "/" . $row->id_stock, "#content", 'Hapus Parsial', 'btn btn-xs btn-danger', 'data-toggle="tooltip" title="Hapus Parsial"');
                                        }
                                        ?>
                                    </td>
                                    <td><?= $row->deskripsi ?></td>
            ...
            <div class="box-footer">
                <?= button_confirm("Anda yakin menyelesaikan stock out?", "wrh/pvc/buat_surat_jalan/" . $id_fppp, "#content", 'Buat Surat Jalan', 'btn btn-success', 'data-toggle="tooltip" title="Buat Surat Jalan"'); ?>
                <?php echo button_confirm("Anda yakin menambahkan item stock out?", "wrh/pvc/additemdetailbom/" . $id_fppp, "#modal", 'Add Item', 'btn btn-info', 'data-toggle="tooltip" title="Add Item"'); ?>
            </div>
            <?php //echo button_confirm("Anda yakin menyelesaikan stock out?", "wrh/pvc/finishdetailbom/" . $id_fppp, "#content", 'Finish', 'btn btn-success', 'data-toggle="tooltip" title="Finish"'); 
            ?>
        </div>
    </div>
</div>
<script type="text/javascript">
    ...
    function divisi(id) {
        ...
        // Send AJAX request
        $.ajax({
            url: "<?= site_url('wrh/pvc/getQtyRowGudang/') ?>",
            dataType: "json",
            type: "POST",
            data: {
                field: fieldname,
            ...
            success: function(response) {
                ...
                // load_silent("wrh/pvc/detailbom/" + $("#id_fppp").val(), "#content");
            }
        })
    }
    ...
    $(".checkbox").change(function() {
        ...
        if (parseInt(qty_gudang) < parseInt(qty_aktual)) {
            ...
            $.ajax({
                url: "<?= site_url('wrh/pvc/saveoutcheck/') ?>",
                dataType: "json",
                type: "POST",
                data: {
                    field: fieldname,
            ...
            if (parseInt(qty_gudang) < parseInt(value)) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // console.log(value);
                        // gudang(edit_id);
                        // load_silent("wrh/pvc/detailbom/" + $("#id_sj").val(), "#content");
                    }
                });
                // }
            }
