    ...
    <div class="col-lg-3">
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Input Stock pvc</h3>
            </div>
...
<script language="javascript">
    function finish() {
        if (confirm('Anda yakin ingin menyelesaikan?')) {
            ...
            load_silent("wrh/pvc/finish_stok_in", "#content");
        }
    }
    ...
    function quotation() {
        ...
        if ($('#aktual').val() != '' && $('#item').val() != '' && $('#qty').val() != '' && $('#id_divisi').val() != '' && $('#id_gudang').val() != '' && $('#keranjang').val() != '') {
            ...
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh/pvc/savestokin') ?>",
                    dataType: 'json',
                    data: {
                        'aktual': $('#aktual').val(),
                        'item': $('#item').val(),
    ...
    function hapus(i) {
        if (confirm('Lanjutkan Proses Hapus?')) {
            $.ajax({
                    type: "POST",
                    url: "<?= site_url('wrh/pvc/deleteItemIn') ?>",
                    dataType: 'json',
                    data: {
                        'id': i
                    }
    ...
    $("select[name=id_gudang]").change(function() {
        $.ajax({
            url: "<?= site_url('wrh/pvc/inOptionGetKeranjang') ?>",
            dataType: "json",
            type: "POST",
            data: {
                "item": $('#item').val(),
